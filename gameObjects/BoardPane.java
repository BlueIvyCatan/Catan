...
//        this.setPrefSize(150,150);
        ...
        this.setStyle("-fx-background-image: url(\"images/background.jpg\");-fx-background-repeat:repeat;");
...
//                    }
    ...
    public void determineHexType(){
        for(int i = 0; i<hexTiles.size(); i++){
            ...
            if(terrain.get(randomNumber).equals("desert")){
    ...
    public void connectHexToCity(){
        for(int i = 0; i<hexTiles.size(); i++){
            ArrayList<Integer> connectedCityIDs = hexCityConnector.get(i);
            for(int k = 0; k < connectedCityIDs.size(); k++){
                hexTiles.get(i).setConnectedCities(cityList.get(connectedCityIDs.get(k)));
